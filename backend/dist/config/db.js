...
exports.ConfigRepository = exports.QuotationsRepository = exports.InquiriesRepository = exports.AdminRepository = void 0;
exports.initializeDatabase = initializeDatabase;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const pg_1 = require("pg");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const JSON_DB_DIR = path_1.default.join(__dirname, '../../database');
const JSON_DB_PATH = path_1.default.join(JSON_DB_DIR, 'db.json');
let pgPool = null;
let useJsonDb = true;
let jsonDbData = {
    admin_users: [],
...
function ensureJsonDbExists() {
    if (!fs_1.default.existsSync(JSON_DB_DIR)) {
        fs_1.default.mkdirSync(JSON_DB_DIR, { recursive: true });
    }
    if (fs_1.default.existsSync(JSON_DB_PATH)) {
        ...
        const defaultPasswordHash = bcryptjs_1.default.hashSync('AdminPass123', salt);
        jsonDbData = {
            admin_users: [
                {
                    id: 1,
...
function writeJsonDb() {
    fs_1.default.writeFileSync(JSON_DB_PATH, JSON.stringify(jsonDbData, null, 2), 'utf8');
}
...
async function seedPgDatabase() {
    ...
    try {
        ...
        if (adminRes.rowCount === 0) {
            const salt = bcryptjs_1.default.genSaltSync(10);
            const passwordHash = bcryptjs_1.default.hashSync('AdminPass123', salt);
        ...
        for (const conf of configs) {
            await client.query('INSERT INTO system_config (key, value, description) VALUES ($1, $2, $3) ON CONFLICT (key) DO NOTHING', [conf.key, conf.value, conf.description || '']);
        }
...
exports.AdminRepository = {
    ...
    async create(username, passwordHash) {
        if (useJsonDb) {
            ...
            const res = await pgPool.query('INSERT INTO admin_users (username, password_hash) VALUES ($1, $2) RETURNING *', [username, passwordHash]);
            return res.rows[0];
        }
    }
};
...
exports.InquiriesRepository = {
    ...
    async create(data) {
        if (useJsonDb) {
            ...
            const res = await pgPool.query('INSERT INTO inquiries (name, email, phone, message) VALUES ($1, $2, $3, $4) RETURNING *', [data.name, data.email, data.phone, data.message]);
            return res.rows[0];
        }
    },
...
exports.QuotationsRepository = {
    ...
    async create(data) {
        if (useJsonDb) {
            ...
            const res = await pgPool.query(`INSERT INTO quotations (
    ...
    async updateStatus(id, status) {
        if (useJsonDb) {
            ...
            const res = await pgPool.query('UPDATE quotations SET status = $1 WHERE id = $2 RETURNING *', [status, id]);
            return res.rows[0] || null;
        }
    },
    async delete(id) {
...
exports.ConfigRepository = {
    ...
    async set(key, value) {
        if (useJsonDb) {
            ...
            await pgPool.query('INSERT INTO system_config (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value', [key, value]);
            return true;
        }
    }
};
