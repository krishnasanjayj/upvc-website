...
import { QuotationsRepository, InquiriesRepository, ConfigRepository } from '../config/db';
...
export async function updateQuoteStatus(req: Request, res: Response) {
  ...
  try {
    const updated = await QuotationsRepository.updateStatus(id, status);
    if (!updated) {
...
export async function exportLeadsToExcel(req: Request, res: Response) {
  try {
